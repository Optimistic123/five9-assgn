...
| Infinite scroll from page 11 on | `src/hooks/useInfiniteScroll.ts`: loads one page per scroll-down gesture near the bottom of the table area. A gesture ends when wheel or trackpad input pauses for 250 ms, or when a new trackpad swipe starts during the previous swipe's momentum. A touch, key press or scrollbar drag each start a new gesture. Scrolling up never loads. At the very bottom, a scroll-down attempt still loads the next page. Safety net: if you're still scrolling down at the bottom 1.5 s after the last page arrived, the next page loads anyway. If the rows don't fill the area, the button stays. |
